...
class ConformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.norm_ff = FQDD_NORMALIZES[layer_norm_type](
            size, eps=norm_eps)  # for the FNN module
        self.norm_mha = FQDD_NORMALIZES[layer_norm_type](
            size, eps=norm_eps)  # for the MHA module
        if feed_forward_macaron is not None:
            self.norm_ff_macaron = FQDD_NORMALIZES[layer_norm_type](
                size, eps=norm_eps)
            self.ff_scale = 0.5
        else:
            self.ff_scale = 1.0
        if self.conv_module is not None:
            self.norm_conv = FQDD_NORMALIZES[layer_norm_type](
                size, eps=norm_eps)  # for the CNN module
            self.norm_final = FQDD_NORMALIZES[layer_norm_type](
