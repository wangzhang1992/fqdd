...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        print('30', src.size(), tgt.size(), src_mask.size(),tgt_mask.size())
        """
        ...
        print('37', src.size(), src_mask.size())
...
def attention(query, key, value, mask=None, dropout=None):
    # torch.Size([10, 8, 1, 64])
    # torch.Size([10, 8, 1, 64])
    # torch.Size([10, 8, 1, 64])
    d_k = query.size(-1)
    # print(d_k)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)
    print('184', scores.size())
    if mask is not None:
        scores = scores.masked_fill(mask == 0, -1e9)
    print('187',scores.size())
    p_attn = F.softmax(scores, dim=-1)
    print('189', p_attn.size())
    if dropout is not None:
        p_attn = dropout(p_attn)
    print('195', p_attn.size()) # torch.Size([10, 8, 2200, 120])
    # torch.Size([10, 8, 1, 64]) value
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout, max_len=2200):
    ...
    def forward(self, x):
        ...
        x = x + torch.squeeze(torch.autograd.Variable(self.pe[:, :x.size(0)], requires_grad=False))
...
print(make_model(1024, 1024))

# batch=16: flops:495656550400.0	params:5432622.0
torch.manual_seed(2021)
src = torch.randn(1, 2200, 120)
src_mask = torch.randint(0, 20, (5 + 5,))
tag = torch.randn(1, 2200, 120)
tag_mask = torch.randint(0, 20, (4 + 5,))
net = make_model(1024, 1024)
input = torch.randn(1, 2200, 120)
flops, params = profile(net, (src_mask, tag_mask, src, tag))
