...
def train(model, train_data, dev_data, configs):
    ...
    if args.pretrained:
        ...
        start_epoch = load_model(os.path.join(args.result_dir, str(args.seed), "save", "AM"), model=model,
                                 optimizer=optimizer, map_location='cuda:{}'.format(args.local_rank))
        # else:
    ...
    for epoch in range(start_epoch, epoch_n + 1):
        ...
        with model_context():
            for idx, data in enumerate(tqdm(train_load)):
                ...
                with context():
                    # 只做推理，代码不会更新模型状态 
                    feats, targets, targets_bos, targets_eos, wav_lengths, target_lens, target_os_lens = [
            ...
            if args.local_rank < 1:
                logger.info("Epoch:{}, loss:{}, cer:{}, lr:{}, corr:{}, det:{}, ins:{}, sub:{}".format(epoch,
                                                                                                       avg_one_epoch_loss,
                                                                                                       cer,
            ...
            with torch.no_grad():
                ...
                logger.info(
                    "DEV:  loss:{}, cer:{}, corr:{}, det:{}, ins:{}, sub:{}".format(loss, cer, corr, det, ins, sub))
...
def evaluate(model, eval_load, ctc_loss, args, phones, device):
    ...
    for idx, data in enumerate(tqdm(eval_load)):
        feats, targets, targets_bos, targets_eos, wav_lengths, target_lens, target_os_lens = [item.to(device) for item
                                                                                              in data]
        # print(feats.shape)
        output_en, output_de = model(feats, targets_bos)
        output_en = torch.nn.functional.log_softmax(output_en, dim=len(output_en.shape) - 1)
...
def main():
    ...
    optimizer, scheduler = init_optimizer(model, configs["model"])

    train(model, train_loader, dev_loader, configs)
