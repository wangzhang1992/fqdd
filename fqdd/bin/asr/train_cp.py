...
from script.models.dense import densenet169
...
def train(model, train_load, dev_load, args, start_epoch, dic, logger, device):
    ...
    for epoch in range(start_epoch, epoch_n):
        ...
        for idx in tqdm(range(len(train_load))):
            ...
            stat_loss += loss.item()
            loss = loss.item()
            if (idx+1)%int(len(train_load)/3) == 0:
                save_model(model, optimizer, epoch, os.path.join(args.result_dir, str(args.seed), 'save'))
            logger.info("batchs:{}   Loss:{:.2f}   CER:{:.2f}  lr:{:.10f}".format(idx+1, loss, cer, optimizer.state_dict()['param_groups'][0]['lr']))
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        device = torch.device(args.cuda_device) #0好GPU
    else:
