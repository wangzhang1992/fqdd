...
class Load_Data:    
    ...
    def dataload(self):
        if os.path.exists(self.train_json_path):
            train_data = My_Data(self.train_json_path, self.phones, feat_type=self.params.feat_type, feat_cof=self.params.feat_cof)
            if self.params.is_distributed:
                self.train_sampler = DistributedSampler(train_data)
                self.train_load = DataLoader(dataset=train_data, sampler=self.train_sampler, batch_size=self.params.batch_size, pin_memory=True, num_workers=self.params.num_workers, collate_fn=self.collate_fn, drop_last=True)
            else:
                self.train_load = DataLoader(dataset=train_data, batch_size=self.params.batch_size, shuffle=self.params.shuffle, pin_memory=True, num_workers=self.params.num_workers, collate_fn=self.collate_fn, drop_last=True)

        if os.path.exists(self.test_json_path):
            test_data = My_Data(self.test_json_path, self.phones, feat_type=self.params.feat_type, feat_cof=self.params.feat_cof)
            self.test_load = DataLoader(dataset=test_data, batch_size=self.params.batch_size, num_workers=self.params.num_workers, shuffle=False, collate_fn=self.collate_fn, drop_last=False)
        if os.path.exists(self.dev_json_path):
            dev_data = My_Data(self.dev_json_path, self.phones, feat_type=self.params.feat_type, feat_cof=self.params.feat_cof)
            self.dev_load = DataLoader(dataset=dev_data, batch_size=self.params.batch_size, num_workers=self.params.num_workers, shuffle=False, collate_fn=self.collate_fn)
