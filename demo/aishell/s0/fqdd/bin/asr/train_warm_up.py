...
def train(model, train_load, dev_load, args, start_epoch, dic, logger, device):
    ...
    num_steps = len(train_load) * (epoch_n - start_epoch + 1)
    ...
    for epoch in range(start_epoch, epoch_n+1):
        ...
        for idx in range(len(train_load)):
            ...
            if (idx+1) %slice_step ==0:
                av_cer_step = sum(tcer[-slice_step:]) / slice_step
                av_loss_step = sum(tloss[-slice_step:]) / slice_step
                logger.info("batchs:{}   Loss:{:.2f}   CER:{:.2f}   lr:{:.10f}".format(idx+1, av_loss_step, av_cer_step, optimizer.state_dict()['param_groups'][0]['lr']))
