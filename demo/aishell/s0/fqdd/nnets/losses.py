...
net = torch.nn.Linear(80, 90).to("cuda:2")
log_probs = net(log_probs)
targets = torch.IntTensor([[2, 10, 14, 24]]).to("cuda:2")
input_lens = torch.FloatTensor([1.0]).to("cuda:2")
target_lens = torch.FloatTensor([1.0]).to("cuda:2")
blank_index = 0
loss = transducer_loss(log_probs, targets, input_lens, target_lens, blank_index)
print(loss)
'''
