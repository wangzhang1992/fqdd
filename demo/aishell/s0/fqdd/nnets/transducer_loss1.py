...
class Transducer(Function):
    ...
    def forward(ctx, log_probs, labels, T, U, blank, reduction):
        ...
        log_probs_m = cuda.to_device(log_probs)
        labels_m = cuda.to_device(labels)
        alpha_m = cuda.to_device(alpha)
        log_p_alpha_m = cuda.to_device(log_p_alpha)
        T_m = cuda.to_device(T)
        U_m = cuda.to_device(U)
        lock_m = cuda.to_device(lock)
        beta_m = cuda.to_device(beta)
        log_p_beta_m = cuda.to_device(log_p_beta)
        grads_m = cuda.to_device(grads)
        ary = np.empty((B,), dtype=np.float32)
        print(log_p_alpha.mean())
        cu_kernel_forward[B, maxU](
            log_probs_m, labels_m, alpha_m, log_p_alpha_m, T_m, U_m, blank, lock_m,
        )
        ...
        cu_kernel_backward[B, maxU](
            log_probs_m, labels_m, beta_m, log_p_beta_m, T_m, U_m, blank, lock_m
        )
        
        cu_kernel_compute_grad[maxT, B](
            log_probs_m, labels_m, alpha_m, beta_m, grads_m, T_m, U_m, blank
        )
        log_p_alpha_m.copy_to_host(ary)
        print(ary)
